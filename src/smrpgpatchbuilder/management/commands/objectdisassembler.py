    ...
    def handle(self, *args, **options):
        ...
        for i in range(len(roomevent_ptrs)):
            if (i < len(roomevent_ptrs) - 1):
                roomevent_lengths.append(
                    roomevent_ptrs[i + 1] - roomevent_ptrs[i])
                ...
                roomevent_raw_data.append(
                    rom[roomevent_ptrs[i]:roomevent_ptrend])
        for i in range(len(roomexit_ptrs)):
            if (i < len(roomexit_ptrs) - 1):
                roomexit_lengths.append(
                    roomexit_ptrs[i + 1] - roomexit_ptrs[i])
                roomexit_raw_data.append(
                    rom[roomexit_ptrs[i]:roomexit_ptrs[i + 1]])
            else:
                ...
                if len(d) == 0 and len(r) == 0 and len(e) == 0:
                    ...
                    if len(r) > 0:
                        ...
                        if len(event_triggers) == 0:
                            ...
                            for j in range(len(event_triggers)):
                                ...
                                writeline(file, '      "length": %i,' %
                                        t["length"])
                                writeline(file, '      "height": %i,' %
                                        t["height"])
                                writeline(
                                    file, '      "nw_se_edge_active": %r,' % t["enable_x_edge"])
                                writeline(file, '      "ne_sw_edge_active": %r,' %
                                        t["enable_y_edge"])
                                writeline(file, '      "byte_8_bit_4": %r,' % t["byte_8_bit_4"])
                    ...
                    if len(e) > 0:
                        ...
                        while j < len(e):
                            ...
                            if (exit_type == 0): #room
                                ...
                                dst_f, _ = byte(prefix="RadialDirection", table=radial_direction_table)([dst_f_val])
                                dst_props = {
                                    "x": field_data[5] & 0x7F,
                                    "y": field_data[6] & 0x7F,
                                    "z": field_data[7] & 0x1F,
                    ...
                    if len(d) > 0:
                        ...
                        while (offset < len(d)):
                            ...
                            assigned_npc = ((d[offset + 4] & 0x0F)
                                            << 6) | (d[offset + 3] >> 2)
                            action_scipt = ((d[offset + 5] & 0x3F)
                                            << 4) | ((d[offset + 4] & 0xFF) >> 4)
                            ...
                            if otype <= 1:
                                event = ((d[offset + 7] & 0x0F)
                                        << 8) | d[offset + 6]
                                writeline(
                                    file, '      "event_script": %i,' % event)
                            elif otype == 2:
                                writeline(file, '      "battle_pack": %i,' %
                                        d[offset + 6])
                                writeline(file, '      "after_battle": %i,' %
                                        (d[offset + 7] & 0x0F))
                            writeline(file, '      "action_script": %i,' %
                                    action_scipt)
                            writeline(file, '      "speed": %i,' % speed)
                            if otype == 0:
                                writeline(file, '      "npc_id_offset": %i,' %
                                        (d[offset + 8] & 0x07))
                                writeline(file, '      "event_offset": %i,' %
                                        (d[offset + 8] >> 5))
                                writeline(file, '      "action_offset": %i,' %
                                        ((d[offset + 8] & 0x1F) >> 3))
                            elif otype == 1:
                                writeline(file, '      "star_offset": %i,' %
                                        (d[offset + 8] & 0x0F))
                                writeline(file, '      "item_offset": %i,' %
                                        (d[offset + 8] >> 4))
                            elif otype == 2:
                                writeline(file, '      "action_offset": %i,' %
                                        (d[offset + 8] & 0x0F))
                                writeline(file, '      "pack_offset": %i,' %
                                        (d[offset + 8] >> 4))
                            writeline(file, '      "visible": %r,' %
                            ...
                            writeline(file, '      "z": %i,' %
                                    (d[offset + 11] & 0x1F))
                            writeline(file, '      "z_half": %r,' %
                                    bit_bool_from_num(d[offset + 10], 7))
                            direction, _ = byte(prefix="RadialDirection", table=radial_direction_table)(
                                [d[offset + 11] >> 5])
                            writeline(file, '      "direction": %s,' % (direction))
                            writeline(file, '      "face_on_trigger": %r,' %
                                    bit_bool_from_num(d[offset + 1], 3))
                            writeline(file, '      "cant_enter_doors": %r,' %
                                    bit_bool_from_num(d[offset + 1], 4))
                            writeline(file, '      "byte2_bit5": %r,' %
                                    bit_bool_from_num(d[offset + 1], 5))
                            writeline(file, '      "set_sequence_playback": %r,' %
                                    bit_bool_from_num(d[offset + 1], 6))
                            writeline(file, '      "cant_float": %r,' %
                                    bit_bool_from_num(d[offset + 1], 7))
                            writeline(file, '      "cant_walk_up_stairs": %r,' %
                                    bit_bool_from_num(d[offset + 2], 0))
                            writeline(file, '      "cant_walk_under": %r,' %
                                    bit_bool_from_num(d[offset + 2], 1))
                            writeline(file, '      "cant_pass_walls": %r,' %
                                    bit_bool_from_num(d[offset + 2], 2))
                            writeline(file, '      "cant_jump_through": %r,' %
                                    bit_bool_from_num(d[offset + 2], 3))
                            writeline(file, '      "cant_pass_npcs": %r,' %
                                    bit_bool_from_num(d[offset + 2], 4))
                            writeline(file, '      "byte3_bit5": %r,' %
                                    bit_bool_from_num(d[offset + 2], 5))
                            writeline(file, '      "cant_walk_through": %r,' %
                                    bit_bool_from_num(d[offset + 2], 6))
                            writeline(file, '      "byte3_bit7": %r,' %
                                    bit_bool_from_num(d[offset + 2], 7))
                            writeline(file, '      "slidable_along_walls": %r,' %
                                    bit_bool_from_num(d[offset + 3], 0))
                            writeline(file, '      "cant_move_if_in_air": %r,' %
                                    bit_bool_from_num(d[offset + 3], 1))
                            writeline(file, '      "byte7_upper2": 0x%02x,' %
                                    (d[offset+5] >> 6))
                            n += 1
                            ...
                            if (extra_length > 0):
                                ...
                                for o in range(offset + 12, offset + l - 1, 4):
                                    ...
                                    if otype == 0:
                                        writeline(
                                            file, '          "npc_id_offset": %i,' % (d[o] & 0x07))
                                        writeline(
                                            file, '          "event_offset": %i,' % (d[o] >> 5))
                                        writeline(file, '          "action_offset": %i,' % (
                                            (d[o] & 0x1F) >> 3))
                                    elif otype == 1:
                                        writeline(
                                            file, '          "star_offset": %i,' % (d[o] & 0x0F))
                                        writeline(
                                            file, '          "item_offset": %i,' % (d[o] >> 4))
                                    elif otype == 2:
                                        writeline(
                                            file, '          "action_offset": %i,' % (d[o] & 0x0F))
                                        writeline(
                                            file, '          "pack_offset": %i,' % (d[o] >> 4))
                                    writeline(file, '          "visible": %r,' %
                                    ...
                                    direction, _ = byte(
                                        prefix="RadialDirection", table=radial_direction_table)([d[o + 3] >> 5])
                                    writeline(
        ...
        file = open("%s/roomobjects.py" % dest,
                    "w", encoding='utf-8')
        for i in range(512):
            writeline(
