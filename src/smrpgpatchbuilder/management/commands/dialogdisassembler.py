...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        file.write("from smrpgpatchbuilder.datatypes.dialogs.classes import Dialog\n".encode("utf8"))
        ...
        for i in range(len(pointers_relative)):
            ...
            if ptr is not None:
                ...
                file.write(('pointers[%i] = Dialog(bank=0x%02x, index=%i, pos=%i)\n' % (i, ptr["bank"], ptr["index"], ptr["pos"])).encode("utf8"))
        file.close()
            
        file = open("%s/dialogs.py" % dest, "wb")
        file.write("from smrpgpatchbuilder.datatypes.dialogs.classes import DialogCollection\n".encode("utf8"))
        ...
        for index, bank in enumerate(banks):
            file.write(f"from .contents.dialog_table_0x{bank["id"]:02x} import dialog_data as data_{index}\n".encode("utf8"))
            args.append(f"data_{index}")
        file.write(f"\ndata = DialogCollection(pointers, [{", ".join(args)}], compression_table={compression_table})".encode("utf8"))
        file.close()
