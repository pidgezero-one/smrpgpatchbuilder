...
from types.overworld_scripts.arguments.types import ByteVar, ShortVar
from types.numbers import Int8, UInt16, UInt8
        ...
        for arg in args:
            if isinstance(arg, (ShortVar, ByteVar)):
                ...
                raise InvalidCommandArgumentException(
                    f"unknown argument type in {self.identifier}: {type(arg)} ({arg})"
                )
        if len(output) != self.size:
            raise RenderException(
                (
                    f"{self.identifier} output wrong length:"
